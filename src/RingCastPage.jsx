...
import { ArrowLeft, PhoneCall, Upload, Send, CheckCircle, Clock, Users, FileText, Calendar, TrendingUp, Sparkles, Zap } from 'lucide-react';
...
function RingCastPage() {
  ...
  const handleSubmit = async () => {
    // Validation
    if (!formData.campaignName || !formData.broadcastMessage || !formData.csvFile) {
    ...
    if (!session || !session.user) {
      setError('Please log in to use RingCast');
      navigate('/login');
      return;
    }

    console.log('🔍 Starting RingCast for:', formData.campaignName);
    ...
    try {
      ...
      // Calculate token cost: 40 tokens per call
      const callCount = totalCalls;
      const totalCost = callCount * 40;

      console.log(`📊 RingCast Cost Calculation:`);
      console.log(`   Calls: ${callCount}`);
      console.log(`   Rate: 40 tokens per call`);
      ...
      const result = await executeWithTokens(
        ...
        async () => {
          ...
          const payload = {
            ...
            broadcast_message: formData.broadcastMessage,
            contacts_csv: csvText
          };

          const response = await fetch(
            'https://glowing-g79w8.crab.containers.automata.host/webhook/ringcast',
            {
              method: 'POST',
        ...
        callCount, // Token multiplier (40 tokens × callCount)
        `Voice Broadcast: ${formData.campaignName} (${callCount} calls)`, // Output summary
      ...
      if (campaignId) {
        ...
        if (!result.logId) {
          ...
          if (agentError) {
            ...
            console.error('❌ RingCast agent not found in database');
          } else {
            const agentId = agentData.id;
            console.log('✅ Agent ID:', agentId);
            
            const outputSummary = `Voice Broadcast: ${formData.campaignName} (${totalCalls} calls)`;
      ...
      setError(err.message || 'Failed to start voice broadcast campaign');
      setLoading(false);
    }
  };

  const simulateCallProgress = () => {
    let count = 0;
    const interval = setInterval(() => {
      count++;
      setCurrentCall(count);
    ...
    }, 300); // Slightly slower than WhatsApp to simulate voice calls
  };

  const resetForm = () => {
    setFormData({ campaignName: '', broadcastMessage: '', csvFile: null });
    setCurrentCall(0);
    setTotalCalls(0);
    setCompleted(false);
    setLoading(false);
          ...
          <>
            ...
            <div style={styles.header}>
              <div style={styles.aiPoweredBadge}>
                ...
                <span style={{color: '#9333ea', fontSize: '0.875rem', fontWeight: '600'}}>AI-POWERED VOICE BROADCASTING</span>
              </div>
              
              <h1 style={styles.title}>RingCast</h1>
              
              <p style={styles.subtitle}>
                Launch automated voice broadcast campaigns with intelligent call distribution and real-time analytics
              </p>

              <div style={styles.connectedBadge}>
                <CheckCircle size={20} style={{color: '#10b981'}} />
                <span style={{color: '#10b981', fontWeight: '600'}}>Voice API Connected</span>
            ...
            <div style={styles.glassCard} className="glass-card">
              ...
              <div style={styles.formGrid}>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    style={styles.input}
                    placeholder="Product Launch Voice Campaign"
                ...
                <div>
                  ...
                  <label style={styles.fileUploadLabel} className="file-upload-label">
                    ...
                    <span style={{marginLeft: '0.75rem'}}>
                      {formData.csvFile ? formData.csvFile.name : 'Upload CSV (Name, Phone)'}
                    </span>
                  </label>
                  {totalCalls > 0 && (
                    <p style={{...styles.helpText, color: '#10b981'}}>
                      <CheckCircle size={16} />
                      {totalCalls} contacts ready to call
                    </p>
                  )}
                  <p style={styles.helpText}>
                    CSV Format: Name, Phone Number (one per line)
                  </p>
                </div>
              </div>
              ...
              <div style={styles.formGroup}>
                ...
                <textarea
                  ...
                  placeholder="Hello {{Name}}, this is an important message about our exclusive offer. We're excited to share that you've been selected for our premium membership program. Visit our website to claim your benefits today!"
                ...
                <p style={styles.helpText}>
                  ...
                  Use {'{{Name}}'} for automatic personalization. Message will be converted to speech.
                </p>
              </div>
              ...
              <p style={styles.bottomText}>
                Calls will be distributed intelligently with optimal timing and delivery rate
              </p>
            </div>
          </>
        )}
            ...
            <div style={styles.glassCard}>
              <div style={styles.progressRing}>
                ...
                <div style={styles.progressRingIcon}>
                  <PhoneCall size={48} style={{color: '#9333ea', animation: 'pulse 1.5s infinite'}} />
                </div>
              </div>

              <h2 style={styles.loadingTitle}>Broadcasting Calls in Progress</h2>
              
              <p style={styles.loadingSubtext}>
                Making personalized voice calls with AI optimization
              </p>

              <div style={styles.counterBox}>
                <div style={styles.counterNumber}>{currentCall}</div>
                <div style={{color: '#9ca3af'}}>of {totalCalls} calls completed</div>
              </div>

              <div style={styles.progressBar}>
                <div style={{
                  ...styles.progressBarFill,
                  width: `${(currentCall / totalCalls) * 100}%`
                }}></div>
              </div>

              <p style={{...styles.helpText, marginTop: '1.5rem', justifyContent: 'center'}}>
                <Clock size={16} />
                Estimated completion: {Math.ceil((totalCalls - currentCall) * 0.3)} seconds
              </p>
            </div>
          </div>
        )}
        ...
        {completed && (
          <div style={styles.successContainer}>
            <div style={styles.successCard}>
              ...
              <h1 style={styles.successTitle}>Voice Campaign Completed!</h1>
              
              <p style={{...styles.subtitle, marginBottom: '2rem'}}>
                Your personalized voice messages have been delivered to all contacts
              </p>

              {/* Stats Grid */}
              <div style={styles.statsGrid}>
                <div style={styles.statCard} className="stat-card">
                  <Users size={32} style={{color: '#9333ea', margin: '0 auto 0.75rem'}} />
                  <div style={styles.statValue}>{totalCalls}</div>
                  <div style={{fontSize: '0.875rem', color: '#9ca3af'}}>Calls Made</div>
                </div>
                ...
                <div style={styles.statCard} className="stat-card">
                  <Zap size={32} style={{color: '#ec4899', margin: '0 auto 0.75rem'}} />
                  <div style={styles.statValue}>{Math.round(totalCalls * 0.3)}s</div>
                  <div style={{fontSize: '0.875rem', color: '#9ca3af'}}>Total Duration</div>
                </div>

                <div style={styles.statCard} className="stat-card">
                  <Calendar size={32} style={{color: '#3b82f6', margin: '0 auto 0.75rem'}} />
              ...
              <div style={styles.detailsCard}>
                <div style={styles.detailsHeader}>
                  <FileText size={24} style={{color: '#9333ea'}} />
